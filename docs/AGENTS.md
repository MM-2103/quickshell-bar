...
| `SystemTheme` | bridges shell theme state to the rest of the desktop. Watches `ThemePresets.currentTheme`; when it changes, runs three external commands as a single chained `sh -c`: `gsettings set color-scheme` (XDG portal — covers GTK4/libadwaita, Qt6 via portal, Electron 14+, Firefox 119+, Chromium-based) + `gsettings set gtk-theme` (legacy GTK3 apps) + `plasma-apply-colorscheme` (KDE/Qt apps via `QT_QPA_PLATFORMTHEME=kde` — Dolphin, Kate, Konsole; gated on `command -v` so non-KDE installs skip silently). One-way (shell → external); disable via `systemThemeSync: false`. Bootstrapped from `shell.qml`'s `Component.onCompleted`. **Debugging recipes** (verify chain, common failure modes, manual reset, the resolved shell command) live in [`docs/CUSTOMIZATION.md`](CUSTOMIZATION.md) under "System theme sync" — start there when an app isn't following a theme switch. |
