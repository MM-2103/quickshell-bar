...
| `ThemePresets` | built-in palette catalogue (20 themes: 10 dark + 10 light) + user-theme directory scan + applyTheme + currentTheme matching + currentSibling + toggleLightDark. The Theme tab in the Settings page renders one card per entry in `ThemePresets.all`; the CC's Theme tile reads `currentSibling` to enable/disable + `toggleLightDark` to apply the sibling. Card click writes all 14 colour keys via Local.set in one debounced batch. |
| `SystemTheme` | bridges shell theme state to the rest of the desktop. Watches `ThemePresets.currentTheme`; when it changes, runs `gsettings set` for `color-scheme` (XDG portal preference) and `gtk-theme`. Apps that respect the portal — GTK4/libadwaita, Qt6, Electron 14+, Firefox 119+, Chromium-based, GTK3 — follow the dark/light preference automatically. One-way (shell → gsettings); user can disable via `systemThemeSync: false`. Bootstrapped from `shell.qml`'s `Component.onCompleted`. |
...
| `themes/` | `ThemePresets` singleton (palette catalogue + apply/current/sibling logic) + `SystemTheme` singleton (gsettings orchestration that propagates the shell's dark/light preference to GTK / Qt6 / Electron / Firefox / Chromium via the XDG portal). The Theme tab in the Settings page surfaces ThemePresets; the CC Theme tile drives toggleLightDark. SystemTheme is bootstrapped from `shell.qml`'s `Component.onCompleted` and reactively syncs to gsettings on every theme change. Disable via `systemThemeSync: false` in `config.jsonc`. |
...
- The CC tile order (Wi-Fi · Bluetooth · Profile / Caffeine · DND · Wallpaper / Theme) — fixed in `controlcenter/TilesView.qml` for muscle-memory stability; reordering requires user sign-off. Cells 8 and 9 (right side of row 3) are reserved for future tiles
- The `Tile.qml` API (`icon`, `brand`, `label`, `stateText`, `active`, `showChevron`, `iconColor`, `enabled`, `clicked`, `chevronClicked`) — used by all 7 tiles. `enabled` is inherited from Item but Tile renders a dimmed visual when false (icon + label + state in `textMuted`); existing tiles that don't set `enabled` keep working unchanged.
