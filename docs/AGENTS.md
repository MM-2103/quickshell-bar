...
| `ThemePresets` | built-in palette catalogue + user-theme directory scan + applyTheme + currentTheme matching. The Theme tab in the Settings page renders one card per entry in `ThemePresets.all`; card click writes all 14 colour keys via Local.set in one debounced batch. |
...
| Add a built-in theme preset | `themes/ThemePresets.qml` (the `builtIn` array) | append a record `{ id, label, palette: { 14 colour keys } }`; `id` must be unique among built-ins; `palette` must populate every key in `paletteKeys` (otherwise that key falls back to Theme.qml's default and applying the theme leaves a half-applied look); update `docs/CUSTOMIZATION.md`'s catalogue table with a one-line entry |
...
| `themes/` | `ThemePresets` singleton — built-in palette catalogue (10 themes shipped) + on-demand scanner for user-defined themes at `~/.config/quickshell-bar/themes/*.jsonc`. Surfaced via the Theme tab in the Settings page; one card click writes all 14 colour keys via `Local.set` (debounced into a single `config.jsonc` flush) |
