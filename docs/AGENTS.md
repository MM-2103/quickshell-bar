...
| `themes/` | `ThemePresets` singleton — built-in palette catalogue (20 themes shipped: 10 dark + 10 light variants paired via `siblingId`) + on-demand scanner for user-defined themes at `~/.config/quickshell-bar/themes/*.jsonc`. Surfaced via the Theme tab in the Settings page; one card click writes all 14 colour keys via `Local.set` (debounced into a single `config.jsonc` flush). Also exposes `currentSibling` + `toggleLightDark()` consumed by the CC's Theme tile for one-click light↔dark variant swaps. |
