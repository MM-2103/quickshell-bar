...
| `activeWindowEnabled` | bool | `true` | Show the focused window's title in the bar, between the workspaces and the clock. Falls back to the app id for clients that set no title, and hides entirely when nothing is focused. Needs a compositor implementing `wlr-foreign-toplevel-management-v1` (Hyprland / niri / Sway all do). |
| `activeWindowMaxWidth` | int (px) | `400` | Upper bound on the title's width. The gap to the clock already bounds it, so this only matters if you want it shorter than the space available; the title elides and hovering shows the full text. |
| `microphoneIndicator` | string | `"auto"` | When the bar's mic indicator is shown. `"auto"` = only while muted or while an application is capturing (doubles as a privacy indicator), `"always"` = like the other bar widgets, `"never"` = hidden. The mic OSD is unaffected either way. |
| `nightLightEnabled` | bool | `false` | Blue-light filter on/off. Normally written by the Control Center's Night light tile rather than by hand. Unlike Caffeine and DND this persists, so the filter survives a shell restart. The `hyprsunset` daemon runs only while this is on, so leaving it off costs nothing at login or on reload. |
