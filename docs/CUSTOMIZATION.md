...
| `nightLightEnabled` | bool | `false` | Blue-light filter on/off. Normally written by the Control Center's Night light tile rather than by hand. Unlike Caffeine and DND this persists, so the filter survives a shell restart. |
